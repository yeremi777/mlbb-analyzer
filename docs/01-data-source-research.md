...
| Official Mobile Legends hero detail page pattern | https://www.mobilelegends.com/hero/detail?channelid=2678742&heroid=6 | Individual hero detail page for manually checking one known hero ID. The `heroid` query parameter identifies the hero, while `channelid` appears to identify the official site channel/page context. May expose official hero name, role, lane, and image data when rendered. | Manual reference and explicit manual-run raw snapshot capture for approved URLs only | Approved only for user-triggered manual-run fetches of explicit reviewed URLs; not approved for scheduled crawling, broad scraping, or ID enumeration | Unknown; terms and attribution still need review | Low to Medium for one-off manual verification; Unknown to Medium for manual-run raw snapshots until terms are reviewed | Correct official detail example for Tigreal. A user-provided inspected Tigreal sample approved creating a manual-run raw fetch script that writes snapshots only under `public/data/raw/official-heroes/`. Do not enumerate `heroid` values, crawl detail pages, schedule the script, bypass source controls, or write fetched data directly to `public/data/heroes.json`. |
